...
public class FeedbackLoop {
    ...
    public void start() {
        ...
        Boolean is_a_drift_scenario = true; // configure with drift scenarios

        Random rand = new Random(150);
        Random drift_rand = new Random(140);
        ...
        for (int i = 1; i <= ConfigLoader.getInstance().getAmountOfCycles(); i++) {
            ...
            Boolean is_drifted = false;
            if(is_a_drift_scenario){
                if(i > 100 && i <= 190){
                    is_drifted = true;
                    Double epsilon = 2.5/90.0;
                    epsilon2 = 8.0 + (i - 100 - 1) * epsilon;
                    epsilon1 = 8.0 + (i - 100) * epsilon;
                }
                else if(i > 190 && i <= 350){
                    is_drifted = true;
                    Double epsilon = 2.0/160.0;
                    ...
                    is_drifted = true;
                    Double epsilon = 1.0/150.0;
                    epsilon2 = 9.0 - (i - 720 - 1) * epsilon;
                    epsilon1 = 9.0 - (i - 720) * epsilon;
                }
                else if(i > 890 && i <= 1070){
                    is_drifted = true;
                    Double epsilon = 3.0/180.0;
            ...
            if(is_drifted){
                if(drift_rand.nextInt(100) > 97){
                    epsilon1 = 8.0;
                    epsilon2 = 11.0;    
                }
                Map<Integer, Double> memory =  ((DoubleRange)deltaiot.DeltaIoTSimulator.simul.getRunInfo().getGlobalInterference()).memory;
                deltaiot.DeltaIoTSimulator.simul.getMoteWithId(12).getLinkTo(deltaiot.DeltaIoTSimulator.simul.getMoteWithId(7)).setSnrEquation(new domain.SNREquation(0.9254, -16.21 * 2));
                deltaiot.DeltaIoTSimulator.simul.getMoteWithId(7).getLinkTo(deltaiot.DeltaIoTSimulator.simul.getMoteWithId(3)).setSnrEquation(new domain.SNREquation(0.5855, -6.644 * 4));
                deltaiot.DeltaIoTSimulator.simul.getMoteWithId(10).getLinkTo(deltaiot.DeltaIoTSimulator.simul.getMoteWithId(5)).setSnrEquation(new domain.SNREquation(0.6463, -3.0037 * 6));
                deltaiot.DeltaIoTSimulator.simul.getRunInfo().setGlobalInterference(new DoubleRange(epsilon1, epsilon2));
                ((DoubleRange)deltaiot.DeltaIoTSimulator.simul.getRunInfo().getGlobalInterference()).memory = memory;
            }
            else{
                Map<Integer, Double> memory =  ((DoubleRange)deltaiot.DeltaIoTSimulator.simul.getRunInfo().getGlobalInterference()).memory;
                deltaiot.DeltaIoTSimulator.simul.getMoteWithId(12).getLinkTo(deltaiot.DeltaIoTSimulator.simul.getMoteWithId(7)).setSnrEquation(new domain.SNREquation(0.9254, -16.21));
                deltaiot.DeltaIoTSimulator.simul.getMoteWithId(7).getLinkTo(deltaiot.DeltaIoTSimulator.simul.getMoteWithId(3)).setSnrEquation(new domain.SNREquation(0.5855, -6.644));
                deltaiot.DeltaIoTSimulator.simul.getMoteWithId(10).getLinkTo(deltaiot.DeltaIoTSimulator.simul.getMoteWithId(5)).setSnrEquation(new domain.SNREquation(0.6463, -3.0037));
                deltaiot.DeltaIoTSimulator.simul.getRunInfo().setGlobalInterference(new DoubleRange(epsilon1, epsilon2));
                ((DoubleRange)deltaiot.DeltaIoTSimulator.simul.getRunInfo().getGlobalInterference()).memory = memory;
            }
